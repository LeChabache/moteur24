						...
						<div class="pf-head">
							<div class="title-style-12">
								...
								<?php if ( is_singular('job_listing') && \MyListing\is_rating_enabled( get_the_ID() ) ): ?>
									<h5 style="    color: #f35a01;font-size: 16px; line-height: 20px; font-weight: 500; margin-bottom: 15px;"><?php _e( 'Votre avis', 'my-listing' ) ?></h5>
								<?php else: ?>
									<h5 style="    color: #f35a01;font-size: 16px; line-height: 20px; font-weight: 500;"><?php _e( 'Ajouter un commentaire', 'my-listing' ) ?></h5>
								<?php endif ?>
							</div>
						</div>
						<div class="pf-body">
							...
							<div class="form-group">
								<label><?php _e( 'Nom', 'my-listing' ) ?></label>
								<input name="author" type="text" value="<?php echo esc_attr( $commenter['comment_author'] ) ?>" required="required" placeholder="<?php echo esc_html__('Votre nom', 'my-listing') ?>">
							</div>
							...
							<div class="form-group">
								...
								<input name="email" type="email" value="<?php echo esc_attr( $commenter['comment_author_email'] ) ?>" required="required" placeholder="<?php echo esc_html__('Email', 'my-listing') ?>">
							</div>
							...
							<?php ob_start(); ?>
							<div class="form-group">
								<label><?php _e( 'Commentaire', 'my-listing' ) ?></label>
								<textarea rows="5" name="comment" required="required" placeholder="<?php echo esc_html__('Entrer un message...', 'my-listing') ?>"></textarea>
							</div>
							...
							<div class="comment-form-cookies-consent md-checkbox">
                                ...
                                <label for="wp-comment-cookies-consent"><?php _e( 'Enregistrez mon nom, mon e-mail pour mon prochain commentaire.', 'my-listing' ) ?></label>
                            </div>
							...
							<button name="submit" type="submit" class="buttons button-2 full-width">
								<?php if ( is_singular('job_listing') && \MyListing\is_rating_enabled( get_the_ID() ) ): ?>
									<?php echo esc_html__('Poster', 'my-listing') ?>
								<?php else: ?>
									<?php echo esc_html__('Poster', 'my-listing') ?>
								<?php endif ?>
							</button>
									...
									if ( $listing && $listing->type && ! $listing->type->multiple_comments_allowed() ):
										...
										<form action="<?php echo esc_url( admin_url('admin-post.php') ); ?>" method="post" enctype="multipart/form-data">
											...
											<?php if ( \MyListing\is_rating_enabled( get_the_ID() ) ): ?>
												<button type="submit" class="buttons button-2 full-width"><?php echo esc_html__('Mettre à jour le commentaire', 'my-listing') ?></button>
											<?php else: ?>
												<button type="submit" class="buttons button-2 full-width"><?php echo esc_html__('Mettre à jour le commentaire', 'my-listing') ?></button>
											<?php endif ?>
										</form>

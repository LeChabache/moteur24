...
?>
<div class="ad-spot header-a"></div>
<div :class="['mobile-tab-'+state.mobileTab,mapExpanded?'map-expanded':'']" class="cts-explore finder-container fc-type-1 <?php echo esc_attr( $data['finder_columns'] ) ?> <?php echo $data['types_template'] === 'dropdown' ? 'explore-types-dropdown' : 'explore-types-topbar' ?>" id="c27-explore-listings" :style="containerStyles">
	...
	<div class="<?php echo $data['template'] === 'explore-2' ? 'fc-one-column min-scroll' : 'fc-default' ?>">
		...
		<div class="finder-listings min-scroll" id="finderListings" :class="( state.mobileTab === 'results' ? '' : 'visible-lg' )" :style="loading?'overflow:hidden':''">
			...
			<div class="no-results-wrapper" v-show="!loading && found_posts === 0">
				<i class="no-results-icon fa fa-oil-can"></i>
				<li class="no_job_listings_found">
					<?php _e( "Aucune annonce pour le moment,<br>essayer d'autres filtres<br>- ou -", 'my-listing' ) ?>
					<a href="#" class="reset-results-27" style="width:50%;margin:auto;margin-top:20px;" @click.prevent="resetFilters($event); getListings('reset', true);">
						<i class="mi refresh"></i>
						<?php _e( 'Réinitialiser', 'my-listing' ) ?>
					</a>
				</li>
			</div>

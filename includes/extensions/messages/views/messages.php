        ...
        <li v-for="message in messages" :class="conversationClass( message )">
            ...
            <div class="delete-confirm-overlay" v-if="isDelete(message)">
                <div class="action-controllers">
                    <p class="delete-msg">
                        <?php esc_html_e('Êtes-vous sûr de vouloir supprimer ce message?', 'my-listing'); ?>
                    </p>
...
<script type="text/html" id="ml-conversation">
    <div id="message-inbox-chat" class="message-inbox-chat">
        ...
        <div :class="{'inbox-chat-contents':true, '_loading': isLoading}">
            <transition name="vopacity">
                <div class="loading-more-messages" v-if="isLoading">
                    <div class="inner">
                        <i class="fa fa-refresh fa-spin"></i> <?php esc_html_e('Chargement de la conversation', 'my-listing'); ?>
                    </div>
                </div>
            </transition>
            ...
            <form @submit.prevent="send">
                <textarea cols="30" :rows="rows" placeholder="<?php esc_html_e('Bonjour.. ', 'my-listing'); ?>" v-model="message" :maxlength="maxLength" @keyup.enter="send($event)" :disabled="disable" id="ml-conv-textarea"></textarea>
        ...
        <div class="delete-confirm-overlay" v-if="isBlockUser()">
            <div class="action-controllers">
                <p v-if="isBlocked()">
                    <?php esc_html_e('Êtes-vous sûr de vouloir débloquer cet utilisateur?', 'my-listing'); ?>
                </p>

                <p v-else>
                    <?php esc_html_e('Êtes-vous sûr de vouloir bloquer cet utilisateur?', 'my-listing'); ?>
                </p>
        ...
        <div class="delete-confirm-overlay" v-if="isDelete(opponent.id)">
            <div class="action-controllers">
                <p><?php esc_html_e('Êtes-vous sûr de vouloir supprimer cette conversation?', 'my-listing'); ?></p>
    ...
    <div id="message-inbox" class="message-inbox">
        <div class="inbox-header">
            ...
            <?php if ( $compose_enabled ): ?>
                <a href="#" class="compose-btn btn-primary" @click.prevent="compose"><?php esc_html_e('Composer', 'my-listing'); ?></a>
            <?php endif ?>
            <div class="clearfix"></div>
        </div>
        <div class="inbox-contents">
            ...
            <div class="inbox-contents empty-inbox" v-else>
                ...
                <p v-else><?php esc_html_e("Vous n'avez aucun message, pour démarrer une conversation, utilisez le bouton composer", 'my-listing'); ?></p>
                <div class="clearfix"></div>
            </div>
            <!-- Pagination -->
            <div class="load-more-msgs" v-if="loadMoreBtn && !loading" @click.prevent="loadMore">
                <?php esc_html_e( 'Charger plus de conversations', 'my-listing' ) ?>
            </div>
            <div class="load-more-msgs" v-if="loadMoreBtn && loading">
                <i class="fa fa-circle-o-notch fa-spin"></i>
                <?php esc_html_e( 'Chargement..', 'my-listing' ) ?>
            </div>
        </div>
        <?php if ( $compose_enabled ): ?>
            <a href="#" class="compose-btn compose-btn-mobile btn-primary" @click.prevent="compose"><?php esc_html_e('Composer', 'my-listing'); ?></a>
        <?php endif ?>
    </div>
</script>
